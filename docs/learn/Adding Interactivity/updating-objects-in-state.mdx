...
상태는 모든 종류의 JavaScript 값을 가질 수 있습니다. 하지만 리액트 상테가 갖고 있는 객체를 직접적으로 변경해서는 안됩니다. 대신, 객체를 업데이트하고 싶을 때는 새 객체를 만들어야(또는 기존에 있는 객체의 복사본을 만들거나) 합니다.
그리고 나서 그 복사본을 사용해 상태를 설정해야 합니다. 

:::note You will learn

- 리액트 상태인 객체를 올바르게 업데이트하는 방법
...
이제 형식이 동작합니다!
각 입력 필드에 각가의 상태변수를 선언하지 않았다는 것을 확인해보세요. 큰 형식을 사용할 때, 한 객체에 모든 데이터를 그룹으로 만들어서 유지하는 것은 매우 편리합니다 - 정확히 업데이트하는 한 말입니다!
...
동적인 값을 가진 특성을 명시하기 위해 객체 선언 안에 `[``]` 괄호를 사용할 수 있습니다. 여기 세개의 다른 이벤트 핸들러 대신 하나만을 사용한 같은 예시가 있습니다. 
...
하지만 리액트에서는, 상태를 불변하는 것으로 취급해야해요! `city`를 변경하기 위해서, (이전의 것에서 데이터를 미리 채워놓은) 새로운 `artwork` 객체를 만들 필요가 있을 것입니다, 그리고 나서 
