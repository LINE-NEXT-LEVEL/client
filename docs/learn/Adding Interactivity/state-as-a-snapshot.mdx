...
이 말인 즉슨, 인터페이스가 이벤트에 반응하기 위해서는 상태를 업데이트 해야 합니다.
...
컴포넌트의 메모리로써 상태는 함수가 반환되면 사라지는 일반적인 변수들과는 다릅니다. 상태는 사실 함수 바깥의 React에서 관리되고 "살아있습니다".
...
~이 `onClick` 핸들러가 포함된 스냅샷에서~ 상태는 0이었기 때문에 `setNumber(number + 1)`이 호출된 이후에도 상태는 계속 0입니다.
...
`setNumber(number + 1)`을 세 번 호출했다 하더라도, ~이번 렌더링~의 이벤트 핸들러에서 `number`는 항상 `0`이므로 상태를 `1`로 세 번 변경한 것과 같습니다.
이것이 이벤트 핸들러의 실행이 끝난 이후에 React는 `number`를 `3`이 아닌 `1`로 그리는 컴포넌트를 렌더링하는 이유입니다.
...
만약 이전 렌더링의 상태 변경 메소드를 사용했다면, alert가 "0"을 보여줄 것을 예상해봃 수 있습니다.
...
~이전 상태의 렌더링~의 `onClick`에서, `number`는 `setNumber(number + 5)`가 호출된 다음이라도 `0`입니다.
...
- `useState`를 호출하면, React는 ~그 렌더링~에 대한 상태 스냅샷을 제공합니다.
...
`setWalk`를 호출하는 것은 ~다음~ 렌더링에서의 상태를 변경할 뿐입니다. 이전 상태로부터 파생된 렌더링에서의 이벤트 핸들러에는 아무 영향을 끼칠 수 없습니다.
