...
때때로, 여러분은 두 컴포넌트의 상태가 항상 함께 변경되기를 원할 것입니다. 그것을 위해서는, 두 컴포넌트에서 상태를 제거하고 공통된 부모 컴포넌트로 옮겨준 후 props로 내려주어야합니다. 이 것은 _상태 올리기_로 알려져 있으며 여러분이 React 코드를 작성할때 가장 자주하게 될 것중 하나입니다.

여러분이 배울 것

- 컴포넌트들에서 올리기로 상태를 공유하는 법
...
**하지만 여러분이 한번에 하나의 panel만 확장되도록 변경하고 싶다고 해보겠습니다.** 이 디자인에서는, 두번째 panel을 확장하는 것은 첫 panel을 접어야합니다. 여러분은 어떻게 할 수 있을까요?
...
`isActive` 상태 변수를 가진 원 `Panel` 컴포넌트는 그의 부모가 panel의 활성 여부에 영향을 줄 수 없기때문에 비제어됩니다.
...
**각각의 유니크한 상태 조각을 위해, 여러분은 컴포넌트가 "소유"하도록 합니다.** 이 원리는 또한 ["하나의 사실 원천"](https://en.wikipedia.org/wiki/Single_source_of_truth)을 가진다고도 알려져 있습니다. 이는 모든 상태가 하나의 장소에 위치한다는 의미가 아닙니다-하지만 각각의 상태 조각들은, 그 정보 조각을 가지는 _특정_ 컴포넌트에 있습니다. 컴포넌트간에 중복된 상태 공유를 대신하여 여러분은 공통 부모로 _올리고_ 필요로 하는 자식들에게 _내려주게_ 됩니다.
...
- 마지막으로, 이벤트핸들러들을 넘겨주어서 자식들이 부모의 상태를 변경할 수 있도록하세요.
