...
**트리 상단의 어딘가에서 자식 컴포넌트가 데이터가 있는지 "물어볼" 수 있는 어떠한 방법이 요구됩니다.**
...
2. 데이터를 필요로하는 컴포넌트에서 context를 **사용합니다**.
3. 데이터를 특정짓는 컴포넌트에서 context를 **제공합니다**.
...
이는 React에게 "이 `<Section>` 내부의 어느 컴포넌트든 `LevelContext`를 요청할 수 있고, `level`을 반환하세요"라고 전달합니다.
...
## 같은 컴포넌트에서 context를 상요하고 제공하기

지금 여전히 각각의 섹션의 `level`을 수동적으로 특정지어야만 합니다.
...
context는 상단의 컴포넌트에서 정보를 읽게 해주었으므로, 각각의 `Section`은 `Section` 상단에서 `level`을 읽을 수 있고 자동으로 `level + 1`을 넘길 수 있습니다.
어떻게 이를 할 수 있는지 아래 방법이 있습니다:
...
그리고 `Section`은 `Section` 내부의 어떤 것이든 "더 깊은" 단계로 만들기 위해 `LevelContext`로 children을 감쌉니다.

:::note
이 예시는 heading level을 사용했습니다. 중첩된 컴포넌트가 context를 덮어쓰는 방법을 시각적으로 보여주기 위해서입니다.
하지만 context는 다른 많은 사용 케이스들이 있습니다.
전체 서브트리에 필요한 어떠한 정보든 context를 사용해 내려줄 수 있습니다: 현재 색깔테마, 현재 로그인된 사용자, 등등
:::
...
`Post` 컴포넌트 내에 있는 `<Heading>`은 가장 가까운 `<Section>`에서 자동으로 수준을 얻는다는 것을 주목하세요:

<Code src="https://codesandbox.io/s/8t0nb9?file=%2FApp.js&utm_medium=sandpack"/>

여러분은 아 예시가 작동하도록 특별한 무언가를 할 필요가 없습니다.
...
context가 작동하는 방식은 [CSS 특징 상속](https://developer.mozilla.org/en-US/docs/Web/CSS/inheritance)을 기억하게 만듭니다.
...
CSS에서, `color`와 `background-color` 같은 다른 특징들은 서로 덮어쓰지 않습니다. 모든 `<div>`의 `color`를 `background-color`에 영향을 주는 것 없이 빨간색으로 설정할 수 있습니다.
...
만약 당신의 컴포넌트가 사소하지 않다면, 12의 컴포넌트로 12의 props를 내려보내는 일은 드문 일이 아닙니다.
...
예를 들어, `posts` props로 데이터를 바로 사용하지 않는 시각 컴포넌트에 데이터를 넘길지도 모릅니다. 예를 들어, `<Layout posts={posts} />`가 있습니다.
...
- 테마: 만약 앱이 사용자가 모양을 변경하도록 허용하는 경우(예시 : 다크모드), 앱의 최상단에 context provider를 넣고 시각적 모양을 조정해야 하는 컴포넌트에 이 context를 사용합니다.
- 현재 계정: 많은 컴포넌트들이 현재 사용자가 로그인되었는지 알 필요가 있을지도 모릅니다. 로그인 여부를 context에 넣는 것은 트리 내부 어디서든 읽을 수 있어 편리합니다.
몇몇 앱들은 동시에 여러 개의 계정을 작동할 수 있돋록 합니다.(예시 : 다른 사용자에게 글을 남기는 것) 이런 경우들에는 다른 현재 계정 값을 갖지는 중첩된 provider 속에 UI의 일부를 감싸는 것이 편리할 수 있습니다.
...
Context는 정적인 값에 제한적이지 않습니다. 다음 렌더링에 다른 값을 보내고 싶다면, 리엑트는 그 값을 읽어들이는 아래의 모든 컴포넌트들을 업데이트할 것입니다!
...
- context는 컴포넌트 아래의 전체 트리에 정보를 제공하는 것을 가능하게 합니다.
- context를 넘기기:
