...
화면 구성요소들은 유저 입력에 반응하여 업데이트됩니다. 예를들어 이미지 갤러리를 클릭하는 것은 열린 이미지를 변경합니다. React에서 시간에 따라 변경되는 데이터를 _state_ 라고 부릅니다. 여러분은 어떤 컴포넌트에도 state를 추가할 수 있으며 필요에 따라 업데이트할 수 있습니다. 이 챕터에서, 여러분은 상호작용을 관리하고 시간 흐름에 따라 다른 출력을 표현하는 컴포넌트를 작성하는 법을 배울 것입니다.
...
React는 여러분이 _event handler_ 를 여러분의 JSX에 추가할 수 있게 해줍니다. Event handler들은 클릭, 마우스 올리기, 입력창 포커싱 과 같은 유저 상호작용에 반응하는 함수들입니다.
...
:::info Ready to learn this topic?
Event handler를 추가하는 방법을 배우려면 [이벤트에 응답하는 방법](https://beta.reactjs.org/learn/responding-to-events)를 읽으세요
:::

## State: 컴포넌트의 메모리

컴포넌트들은 종종 상호작용의 결과로 화면상에 표시되는 것을 바꿀 필요가 있습니다.
입력창에 타이핑할때 입력 영역이 업데이트되어야하고, 이미지 카라셀의 "다음"을 클릭했을 때 표시되는 이미지를 변경하는 것, "구매" 버튼을 클릭해 장바구니에 물건을 추가하는 것이 있을 것 입니다. 컴포넌트는 현재 입력 값, 현재 이미지, 장바구니를 "기억"해야합니다. 이와 같은 컴포넌트마다 구체화된 메모리를 _state_ 라고 부릅니다.
...
:::info Ready to learn this topic?
[State: 컴포넌트의 메모리](https://beta.reactjs.org/learn/state-a-components-memory)를 읽고 상호작용에 value를 기억하고 업데이트하는 방법을 배워보세요.
:::

## 렌더링과 커밋

여러분의 컴포넌트가 스크린에 표현되기 전에, 컴포넌트들은 React에 의해 렌더링되어야합니다. 이 과정에서 단계를 이해하는 것은 여러분의 코드가 어떻게 동작하고 그 동작을 설명하는지에 대해 생각하는 것에 도움이 될 것 입니다.
...
3. Dom에 커밋하기(테이블에 주문한 것을 올림)

![image](https://user-images.githubusercontent.com/15559593/189927121-6885275f-61c3-42b0-8fd3-c05009b53b19.png)

:::info Ready to learn this topic?
[렌더링과 커밋](https://beta.reactjs.org/learn/render-and-commit)을 읽고 UI 업데이트의 라이프 사이클을 배워보세요.
:::
...
:::info Ready to learn this topic?
[스냅샷과 같은 state](https://beta.reactjs.org/learn/state-as-a-snapshot)를 읽고 왜 state가 event handler 내부에서 왜 바뀌지 않고 "고정된" 것으로 표현되는지 배워보세요.
:::
...
:::info Ready to learn this topic?
[일련의 state 변경 저장하기](https://beta.reactjs.org/learn/queueing-a-series-of-state-changes)를 읽고 다음 렌더링 전에 여러 업데이트를 저장하는 방법을 학습해보세요.
:::
...
:::info Ready to learn this topic?
[State의 객체들을 업데이트하기](https://beta.reactjs.org/learn/updating-objects-in-state)를 보고 객체를 업데이트하는 방법을 정확히 배워보세요.
:::
...
:::info Ready to learn this topic?
[state에서 배열 업데이트하기](https://beta.reactjs.org/learn/updating-arrays-in-state)를 보고 배열을 업데이트하는 방법을 정확히 배워보세요.
:::
